...
PART_1_TEST_B_EXPECT : aoc.Result = 19;
PART_1_TEST_C_EXPECT : aoc.Result = 226;
...
part_1 :: proc($MODE : aoc.LogicMode) -> aoc.Result
{
  ...
  traverse :: proc(links : map[string][dynamic]string, node_index : map[string]int, index_to_node : []string, node : string, visitedNodes : NodeSet, total : ^int)
  {
    if node == "start"
    {
      return;
    ...
    {
      traverse(links, node_index, index_to_node, goto, visitedNodes, total);
    }
  }
  ...
  {
    traverse(links, node_index, index_to_node, goto, {}, &pathes);
  }
