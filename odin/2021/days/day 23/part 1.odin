...
PART_1_TEST_A_EXPECT : aoc.Result = 12521;
...
part_1 :: proc($MODE : aoc.LogicMode) -> aoc.Result
{
  ...
  { // TESTS MODE
    DEBUG  :: true;
    TIMING :: false;
    START_STATE :: State{ { {'B','A'}, {'C','D'}, {'B','C'}, {'D','A'} }, {}, 0 }
  }
  else
  { // SOLVE MODE
    DEBUG  :: false;
    TIMING :: false;
    START_STATE :: State{ { {'C','B'}, {'D','A'}, {'D','B'}, {'A','C'} }, {}, 0 }
  }
  ...
  for {
    ...
    if stacks[0][0] == 'A' && stacks[0][1] == 'A' &&
       stacks[1][0] == 'B' && stacks[1][1] == 'B' &&
       stacks[2][0] == 'C' && stacks[2][1] == 'C' &&
       stacks[3][0] == 'D' && stacks[3][1] == 'D'
    {
      return energy_used
    }
    ...
    for h in 0..10 {
      ...
      for s in 0..3 {
        hmin := min(h, 2*s+2)
        hmax := max(h, 2*s+2)
        if (stacks[s][0] != 0 || stacks[s][1] != 0) && ((stacks[s][0] != 0 && stacks[s][0] != u8(s)+'A') || stacks[s][1] != u8(s)+'A') && is_empty(hall[hmin:hmax+1]) {
          new_state := state
          dist := hmax-hmin
          if stacks[s][0] != 0 {
          ...
          if new_state.energy_used < 13000 {
    ...
    for h in 0..10 {
      ...
      if hall[h] != 0 {
        ...
        hmin := min(h+1, hs)
        hmax := max(h-1, hs)
        if (stacks[s][1] == 0 || (stacks[s][0] == 0 && stacks[s][1] == s+'A')) && is_empty(hall[hmin:hmax+1]) {
          new_state := state
          dist := hmax-hmin+1
          if stacks[s][1] == 0 {
            new_state.hall[h] = 0
